...
import torch
from demo import crop
# from demo2 import loop
import yaml
...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3):
    ...
    img = cv2.rectangle(im, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    db = client['a2']
    ...
    if (a[0] == 'person'):
        plate = crop(int(x[0]),int(x[1]),int(x[2]),int(x[3]),img)
    if (plate != "NO LICENSE PLATE"):
        result = db.license_collection.find_one({"license": plate})
        if(result is None):
            now = datetime.now()
            current_time = now.strftime("%H:%M:%S")
            db.license_collection.insert_one({"license": plate,"time":current_time})
        else: 
            return "done"
    else:
        print("")
